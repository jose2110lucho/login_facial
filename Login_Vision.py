...
from tkinter import * # libreria para realizar la interfaz grafica
import os # libreria para importar archivos y realizar comparaciones
import cv2 # libreria para hacer procesamiento de imagenes
from matplotlib import pyplot # libreria para hacer procesamiento de imagenes
from mtcnn.mtcnn import MTCNN # libreria para hacer deteccion de rostros
import numpy as np # libreria para hacer operaciones con matrices
...
#--------------------------- Funcion para almacenar el registro facial(PASO#5) --------------------------------------
...
def registro_facial():
    ...
    #----------------- Detectamos el rostro y exportamos los pixeles(PASO#6) --------------------------
    
    def reg_rostro(img, lista_resultados): #funcion que dibuja la imagen con los objetos detectados
        data = pyplot.imread(img) # leemos la imagen con la libreria matplotlib
        #vamos a exportar los pixeles que pertenecen a los rostros con el fin de usarlos en otro sistema
    ...
    img = usuario_img+".jpg"
    pixeles = pyplot.imread(img) #leemos la imagen con la libreria matplotlib
    detector = MTCNN() 
    caras = detector.detect_faces(pixeles) #pasamos la imagen al detector de caras
    reg_rostro(img, caras)   
    
#------------------------Crearemos una funcion para asignar al boton registro(PASO#3) --------------------------------
def registro(): # funcion que obtiene la imagen, el usuario y la contraseña
    global usuario
    ...
    #--------- Empezaremos a crear las entradas(PASO#4) ----------------------------------------
...
#--------------------------Funcion para el Login Facial(PASO#8) --------------------------------------------------------
def login_facial():
#------------------------------Vamos a capturar el rostro-----------------------------------------------------
    cap = cv2.VideoCapture(0)               #Elegimos la camara con la que vamos a hacer la deteccion
    while(True):
    ...
    #-------------------------- Funcion para comparar los rostros(PASO#9) --------------------------------------------
    ...
    #---------------------------- Importamos las imagenes y llamamos la funcion de comparacion(PASO#10) ---------------------------------
...
#------------------------Funcion que asignaremos al boton login(PASO#7) -------------------------------------------------
...
#------------------------- Funcion de nuestra pantalla principal(PASO #1) ------------------------------------------------
...
def pantalla_principal():
    ...
    pantalla.title("Login Facial")       #Asignamos el titulo de la pantalla
...
#------------------------- Vamos a Crear los Botones(PASO #2) ------------------------------------------------------
    
    Label(text = "").pack()  #Creamos el espacio entre el titulo y el primer boton
    Button(text = "Iniciar Sesion", bg="grey", fg ="white", height = "2", width = "30", command = login).pack()
    Label(text = "").pack() #Creamos el espacio entre el primer boton y el segundo boton
    Button(text = "Registro", bg="grey", fg ="white", height = "2", width = "30", command = registro).pack()
